...
class AppStore extends Component {
  ...
  filterList = searchedApps => {
    const {activeId} = this.state
    const getFilterList = searchedApps.filter(
      each =>
  ...
  getSearchApps = () => {
    ...
    const filterSearchApps = appsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return filterSearchApps
  }
  render() {
    const getSearchResults = this.getSearchApps()
    const filterdList = this.filterList(getSearchResults)
    const {activeId} = this.state
    return (
